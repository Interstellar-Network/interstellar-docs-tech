---
sidebar_label: 'Signer Orchestrator (delivered in M3)'
sidebar_position: 2
---
...
The Signer Orchestrator replaces the originally planned **sidechain registry pallet** that was envisioned to manage a dedicated signature sidechain cluster per cryptographic scheme. This strategic adjustment reflects a shift in focus from early L1-L2 orchestration toward greater flexibility at the **worker level**.
...
This approach also increases compatibility with future infrastructure upgrades — including the introduction of **TEE/MPC models** and potentially **other MPC models and technologies** — while preserving functional parity and testnet realism during the current phase.
...
The Signer Orchestrator serves as a control point between:
...
The Signer Orchestrator introduces a programmable abstraction layer that separates signature policy from implementation details. It will become essential in enabling a secure, scalable, and extensible multi-chain wallet infrastructure, where different users, coins, and environments may require distinct validation and signature handling pathways.